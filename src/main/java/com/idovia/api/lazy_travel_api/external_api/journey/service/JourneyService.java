...
public class JourneyService implements JourneyInterface {
    ...
    public CompletableFuture<List<JourneyModel>> findAllJourneyBestMatch(CityModel cityDeparture, CityModel cityArrival, String date, String hour, int maxTimeTravel) throws ParseException {
        ...
        return CompletableFuture.completedFuture(ticketsreduce.stream().map(k -> new JourneyModel("train", k.getDeparture_name(), k.getDeparture_date(), k.getDuration(), Double.parseDouble(k.getPrice()), k.getArrival_name(), k.getArrival_date())).collect(Collectors.toList()));
    }
