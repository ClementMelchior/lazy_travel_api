...
import com.idovia.api.lazy_travel_api.external_api.journey.model.KelbilletResponseModel;
...
public class JourneyService implements JourneyInterface {
    ...
    public List<TicketKelbilletModel> findAllJourney (CityModel cityDeparture, CityModel cityArrival, String date, String hour, int maxTimeTravel) throws ParseException {
    ...
    @Async
    public CompletableFuture<List<JourneyModel>> findAllJourneyBestMatch(CityModel cityDeparture, CityModel cityArrival, String date, String hour, int maxTimeTravel) throws ParseException {
        System.out.println("Finding train ticket");
        List <TicketKelbilletModel> tickets = findAllJourney(cityDeparture, cityArrival, date, hour, maxTimeTravel);
        ...
        return CompletableFuture.completedFuture(ticketsreduce.stream().map(k -> new JourneyModel("train", k.getDeparture_name(), k.getDeparture_date(), k.getDuration(), Double.parseDouble(k.getPrice()), k.getArrival_name(), k.getArrival_date(), k.getDeepLink())).collect(Collectors.toList()));
    }

    public List<TicketKelbilletModel> findAllTicketKelbillet (String url) throws ParseException {
    ...
    public boolean isUnderMaxTimeTravel(TicketKelbilletModel t, int maxTimeTravel) throws ParseException {
        SimpleDateFormat format=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");  

        return (format.parse(t.getArrival_date()).getTime() - format.parse(t.getDeparture_date()).getTime())<=(maxTimeTravel*60000);
    }

    public boolean isNotTooLate(TicketKelbilletModel t, String date, String hour) throws ParseException {
    ...
    public String getCustomUrlSncf (String codeCityDeparture, String date, String hour, String codeCityArrival) throws ParseException {
        SimpleDateFormat format=new SimpleDateFormat("yyyy-MM-dd HH:mm");  
    ...
    public String getCustomUrlTictatrip (String codeCityDeparture, String date, String hour, String codeCityArrival) throws ParseException {
